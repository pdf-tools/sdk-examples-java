...
This kit contains the ObfuscateEBill sample for Pdftools SDK for Java. Pdftools SDK is a development library that lets you integrate PDF processing into your applications. For more information, review the Pdftools [documentation portal](https://www.pdf-tools.com/docs/).
...
3. Right-click the file containing the `main` method and select **Run 'PdfToolsObfuscateEBill'**.
...
### Using Maven
  ...
  ```bash
  mvn clean install
  mvn exec:java -Dexec.mainClass="PdfToolsObfuscateEBill.PdfToolsObfuscateEBill" -Dexec.args="<inputPath> <outputPath>"
  ```

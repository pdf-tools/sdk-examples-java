...
This kit contains the BuiltInCertify sample for Pdftools SDK for Java. Pdftools SDK is a development library that lets you integrate PDF processing into your applications. For more information, review the Pdftools [documentation portal](https://www.pdf-tools.com/docs/).
...
3. Right-click the file containing the `main` method and select **Run 'PdfToolsBuiltInCertify'**.
...
### Using Maven
  ...
  ```bash
  mvn clean install
  mvn exec:java -Dexec.mainClass="PdfToolsBuiltInCertify.PdfToolsBuiltInCertify" -Dexec.args="<certificateFile> <password> <inputPath> <outputPath>"
  ```
...
### Using Gradle
  ...
  ```bash
  gradle build
  ./gradlew run --args="<certificateFile> <password> <inputPath> <outputPath>"
  ```
